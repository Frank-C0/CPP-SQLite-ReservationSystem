...
int Reserva::n_id_reserva = 0;
int Reserva::n_dni = 1;
int Reserva::n_paciente = 2;
int Reserva::n_especialidad = 3;
int Reserva::n_doctor = 4;
int Reserva::n_fecha = 5;
int Reserva::n_hora = 6;
...
QString CReservSystem::getDoctor(QString especialidad){
    ...
    return "no se encontro doctor";
}
QString CReservSystem::getHoraReserva(QString, QString){
    return "8:00";
}
bool CReservSystem::createReserva(Reserva r){
    QSqlQuery query2;
    query2.prepare("INSERT INTO reserva (dni, paciente, especialidad, doctor, fecha, hora) VALUES (:dni,:paciente,:especialidad,:doctor,:fecha,:hora);");
    ...
    //query2.bindValue(":hora",           CReservSystem::getHoraReserva(r.especialidad_reserva,r.fecha));
...
std::list<Reserva> CReservSystem::readAllReservas(){
    ...
    QSqlQuery query3("SELECT * FROM reserva;");
       ...
       while (query3.next())
       {
           ...
           // qDebug() << query3.value(Reserva::n_id_reserva).toString();
           // qDebug() << query3.value(Reserva::n_dni).toString();
           // qDebug() << query3.value(Reserva::n_paciente).toString();
           // qDebug() << query3.value(Reserva::n_especialidad).toString();
           // qDebug() << query3.value(Reserva::n_doctor).toString();
           // qDebug() << query3.value(Reserva::n_fecha).toString();
           // qDebug() << query3.value(Reserva::n_hora).toString();
           // qDebug() << "------ -- --  -    -- -----"<<lista_reservas.size();
...
std::list<Reserva> CReservSystem::searchReservasByDniEspecialidaFecha(QString dni,QString especialidad,QString fecha){
    ...
    QSqlQuery query3("SELECT * FROM reserva WHERE dni LIKE '"+dni+"%' AND especialidad LIKE '"+especialidad+"%' AND fecha LIKE '"+fecha+"%';");
    std::list<Reserva> lista_reservas;
